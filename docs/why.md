...
`go-ruby-actioncable/actioncable` reimplements Ruby's `actioncable` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-actioncable/actioncable` directly, with no Ruby runtime;
...
Because `actioncable` is pure Go with cgo disabled, it also compiles to **WebAssembly**
